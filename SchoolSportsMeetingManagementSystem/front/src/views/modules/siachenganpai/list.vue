...
<script>
    ...
    //当前表下拉框搜索条件
        ...
        created() {
            ...
            this.loadUserPermissions(); // 加载权限（关键新增）
        },
        mounted() {
            //获取当前登录用户的信息
        ...
        methods: {
            ...
            // 加载用户权限（关键新增）
            loadUserPermissions() {
                // 实际项目中应从后端接口获取，这里为示例从本地存储获取
                const storedPermissions = this.$storage.get("userPermissions") || [
                    "siachenganpai:新增",  // 确保新增权限存在
                    "siachenganpai:删除",
                    "siachenganpai:查看",
                    "siachenganpai:报表",
                    "siachenganpai:导入导出",
...
.slt {
  ...
  // 条纹行样式 - 删除冲突的透明背景设置
  ::v-deep .el-table__body tr.el-table__row--striped {
